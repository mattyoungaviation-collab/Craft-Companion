...
export const craftWorldWalletLogin = async(body:{payload:any;signature:string;thirdwebSession?:string}) => { const d=await req('/api/auth/craftworld-wallet/login',{method:'POST',body:JSON.stringify(body)}); localStorage.setItem('token', d.token); localStorage.setItem('me', JSON.stringify(d.user)); return d; };
...
export const walletLogin = async(body:{address:string;message:string;signature:string}) => { const d=await req('/api/auth/wallet/login',{method:'POST',body:JSON.stringify(body)}); localStorage.setItem('token', d.token); return d; };
